"""Weight-tied recursive hybrid transformer, GDN:GQA = 3:1, R=3 per super-block.

TWO independent super-blocks in sequence, each a single (gdn, gdn, gdn, gqa)
ratio unit of 4 unique layers. Forward applies each super-block R=3 times
...
n_super_blocks * n_layers = 8 unique layers; effective depth =
n_super_blocks * R * n_layers = 24.
"""
...
Config = importlib.import_module("src.recursion_gdn.3to1.config").Config
